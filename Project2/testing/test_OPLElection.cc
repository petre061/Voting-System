...
TEST_F(fixture_OPLElection, opl_example) {
  OPLElection test("ballot_files/opl_example.csv");

  ASSERT_EQ(test.run(), 0);
}
TEST_F(fixture_OPLElection, multiple_files) {
  OPLElection test(std::vector<std::string>{
...
TEST_F(fixture_OPLElection, OPLElection3waytie) {
  // Test whether each of the 3 parties wins the tie an approximately equal
  // number of times
  ...
  for (int i = 0; i < 10; i++) {
    ...
    // Redirect cout stream to a buffer, collecting output which lists who won
    // how many seats
